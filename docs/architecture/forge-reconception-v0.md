...
| D2-R02 [C1] | Interpret team environment [C1] | TIBER-Teamstate [C1] | **TIBER-Teamstate**, for team-environment semantics and auditable team-level profiles with provenance, coverage, explicit unknowns, and no fabricated market lane. [T01][F01] | `verified_historical_at_freeze`; corroborated by Forecast's current boundary document. [T01][F01] | The Teamstate contract is `historical_implementation_evidence`, not promotion, runtime-conformance, or human-approved future doctrine. Forecast's canonical document establishes only Forecast's declared consumption boundary. [T01][F01] | Exact attachment, readiness, and use by a FORGE successor are `unresolved — D3 input`. [T01] |
| D2-R03 [C1] | Interpret player role and opportunity [C1] | Role-and-opportunity-model [C1] | **Role-and-opportunity-model**, for player-role/opportunity semantics, evidence, confidence, and readiness. It consumes Teamstate context read-only instead of duplicating team-environment logic. [P01][F01] | `verified_historical_at_freeze`; corroborated by Forecast's current boundary document. [P01][F01] | The contract is `historical_implementation_evidence`, contract-first, and not a complete export pipeline. V0 is WR/TE-only; RB is future and QB is excluded. [P01] | Exact FORGE input/readiness treatment and uncovered position semantics are `unresolved — D3 input`. [P01] |
| D2-R04 [C1] | Maintain prospect / transition priors [C1] | TIBER-Rookies [C1] | **TIBER-Rookies**, for the Rookie Alpha producer contract and bounded rookie-transition evidence semantics. Transition evidence separates pre-draft market proxy from observed post-draft outcome and is not itself an outcome forecast. [R01][R02] | `verified_current_at_freeze` for the producer contract; `verified_historical_at_freeze` for the transition-profile contract. [R01][R02] | The export contract is canonical only for TIBER-Rookies' producer output. The transition contract is `historical_implementation_evidence`; downstream adoption and its in-file promotion claim are not elevated by D1. [R01][R02] | Which fields are observations, priors, or admissible successor inputs is `unresolved — D3 input`. [R01] |
...
- Bounded current allocations: observations/governance to TIBER-Data plus
  domain producers; team environment to TIBER-Teamstate; role/opportunity to
  Role-and-opportunity-model; prospect/transition producer semantics to
  TIBER-Rookies; outcome forecasts/uncertainty to TIBER-Forecast; product
  presentation to TIBER-Fantasy. [D01][D02][T01][P01][R01][R02][F01][Y04]
- Reserved for D3: canonical cross-source synthesis, future evaluation-policy
  ownership, complete agent-context ownership, and all successor topology.
  [C1][O30]
...
| P01 | `Prometheus-Frameworks/Role-and-opportunity-model:docs/contracts/role-opportunity-profile-v0.md` @ `6435d8d3c2c4e53dc45ab57a05a2716e2b47598d` | `a156ff5e9f7d335a3a45e76dda8765aaa4883b158c62bc4da02eb9cf4d634bdb` | `historical_implementation_evidence`; committed role/opportunity contract semantics, not promotion, runtime conformance, or future architecture. |
